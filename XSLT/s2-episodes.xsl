...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 and output a single HTML file for each episode, pairing it up with whatever issues files (if any) are associated with it. -->  
    ...
    <xsl:variable name="episodeColl" as="document-node()+" select="collection('../XML/tv/season-2/?select=*.xml')"/>
                    ...
                    <body>
                        <section class="container"> 
                            ...
                            <div id="episodes">  
                                ...
                                <ul id="episodeNav">
                                    <xsl:if test="$episodeNumber gt 1"><li><a href="s2_episode-{$episodeNumber - 1}.html">⇦  Previous episode</a></li></xsl:if>
                                    <xsl:if test="$episodeNumber lt 13"> 
                                        <li><a href="s2_episode-{$episodeNumber + 1}.html">Next episode ⇨</a></li></xsl:if>
                                </ul>
    ...
    <xsl:template match="spkr[@ref='BeastBoy']">
        <span class="beastBoy">Beast Boy<xsl:text>: </xsl:text></span>
    </xsl:template>
